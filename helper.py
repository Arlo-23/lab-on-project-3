...
def data_over_time(df, col):
    ...
    nations_over_time.columns = ['Edition', col]  # Rename columns explicitly
    nations_over_time.sort_values('Edition', inplace=True)  # Sort by 'Edition'
    return nations_over_time


def most_successful(df, selected_sport):
    temp_df = df[df['Sport'] == selected_sport].dropna(subset=['Medal'])

    # Get value counts and reset index
    x = temp_df['Name'].value_counts().reset_index()
    x.columns = ['Name', 'Medals']  # Rename columns to avoid confusion
...
def most_successful_countrywise(df, country):
    ...
    # Merge with the original DataFrame to get additional info
